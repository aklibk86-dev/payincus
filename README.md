...
- 用户后台：注册登录、控制台、套餐市场、实例详情、终端 WebSocket、工单、公告、通知、邀请、钱包、礼品卡、邮箱和托管节点。
- 管理后台：用户、套餐、节点、镜像、订单、日志、OAuth、Telegram、邮件、系统配置、资源池、SLA、告警、统计和版本更新。
- 计费账务：余额、充值、支付回调、消费记录、对账、返利、积分、VIP 等级、会员福利和礼品卡兑换。
...
礼品卡用于把余额或后台发放额度转换为一次性充值兑换码，用户端入口为 `/gift-cards`，管理后台入口为 `/admin/gift-cards`。

能力边界：

- 用户可以用自己的余额生成礼品卡，系统会在同一事务内扣减余额、写入余额流水并创建礼品卡。
- 用户可以兑换他人发放或后台生成的礼品卡，不能兑换自己生成的礼品卡。
- 后台可以单张或批量生成礼品卡，查看统计，启用、禁用或删除未使用礼品卡。
- 后台列表默认只显示脱敏兑换码，只有显式勾选显示完整码时才返回完整码。
- 生产环境必须配置 `PAYINCUS_GIFT_CARD_ADMIN_IDS`，未配置时后台礼品卡管理默认拒绝，避免所有管理员都能直接生成余额类兑换码。
- OTA 升级会通过 `prisma migrate deploy` 创建 `gift_cards` 表；升级前建议先设置管理员白名单，升级后再开放后台入口。

推荐生产环境变量：

```env
PAYINCUS_GIFT_CARD_ADMIN_IDS=1
```
